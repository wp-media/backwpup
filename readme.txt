...
* Security fix: Resolved a Cross-Site Scripting (XSS) vulnerability on the Restore page.
* Fix: Fixed an incorrect backup aborted notice and a page freeze when using the Backup Now button.
* Fix: Fixed an issue affecting the "Don't show again" admin notice action.
...
* Enhancement: Added the backwpup_mcp_server_enabled filter to allow disabling MCP functionality.
* Fix: MCP backup history now reports correct file sizes across all storage destinations.
* Fix: Resolved a WooCommerce compatibility issue that could generate log errors.
* Fix: Resolved PHP 8.1+ compatibility issues affecting MCP operations.
...
* Fix: Addressed a Block Editor List View issue that caused some block names to appear blank.
...
* New Feature: Introduced support tool chatbot for user assistance.

= 5.6.4 =
Release date: January 20, 2026

* Fix: Plugin update mechanism not working properly for Pro version since 5.6.2
...
* Enhancement: Introduced WP-CLI support for running backups
* Enhancement: Improved Mixpanel event tracking accuracy
* Added support for triggering backups via link
* Fix: Resolved a compatibility issue with Visual Composer
...
* Bugfix: Log notification emails were not sent for successful backup
* Removed: Remove unused/deprecated codes
* Enhancement: Refactor the plugin admin notice code
* Bugfix: Save button is not clickable on low browsers window height
* Bugfix: Google Drive & OneDrive API fields not fully visible
* Bugfix: Encryption “Save” button not visible below Asymmetric Key
* Bugfix: Decryption key needed for free plugin
* Enhancement: Storage deselect ux issues

= 5.3.1 =
Release date: July 22, 2025

* Bugfix: Default archive format resets to Tar format after update to 5.3
* Bugfix: Updating to 5.3 causes Fatal error due to corrupted data
* Bugfix: License activation fails due to service provider issue
* Bugfix: Upload to FTP backup limit issue on 5.3
