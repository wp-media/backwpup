...
class BackWPup_JobType_WPEXP extends BackWPup_JobTypes {
	...
	public function job_run( &$job_object ) {
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['SAVE_STEP_TRY'] != $job_object->steps_data[ $job_object->step_working ][ 'STEP_TRY' ] ) {
			...
			$job_object->steps_data[ $job_object->step_working ]['wpexportfile'] = $job_object->generate_filename( $job_object->job[ 'wpexportfile' ], 'xml', TRUE );
		...
		$handle = fopen( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'], 'ab' );
		if ( $handle === FALSE ) {
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'header' ) {
			...
			fwrite( $handle, '<?xml version="1.0" encoding="' . get_bloginfo('charset') . "\" ?>\n" );
			fwrite( $handle, "<!-- This is a WordPress eXtended RSS file generated by the WordPress plugin BackWPup as an export of your site. -->\n" );
			fwrite( $handle, "<!-- It contains information about your site's posts, pages, comments, categories, and other content. -->\n" );
			fwrite( $handle, "<!-- You may use this file to transfer that content from one site to another. -->\n" );
			fwrite( $handle, "<!-- This file is not intended to serve as a complete backup of your site. -->\n\n" );
			fwrite( $handle, "<!-- To import this information into a WordPress site follow these steps: -->\n" );
			fwrite( $handle, "<!-- 1. Log in to that site as an administrator. -->\n" );
			fwrite( $handle, "<!-- 2. Go to Tools: Import in the WordPress admin panel. -->\n" );
			fwrite( $handle, "<!-- 3. Install the \"WordPress\" importer from the list. -->\n" );
			fwrite( $handle, "<!-- 4. Activate & Run Importer. -->\n" );
			fwrite( $handle, "<!-- 5. Upload this file using the form provided on that page. -->\n" );
			fwrite( $handle, "<!-- 6. You will first be asked to map the authors in this export file to users -->\n" );
			fwrite( $handle, "<!--    on the site. For each author, you may choose to map to an -->\n" );
			fwrite( $handle, "<!--    existing user on the site or to create a new user. -->\n" );
			fwrite( $handle, "<!-- 7. WordPress will then import each of the posts, pages, comments, categories, etc. -->\n" );
			fwrite( $handle, "<!--    contained in this file into your site. -->\n\n" );
			fwrite( $handle, "<!-- generator=\"WordPress/" . get_bloginfo_rss('version') . "\" created=\"". date('Y-m-d H:i') . "\" -->\n" );
			fwrite( $handle, "<rss version=\"2.0\" xmlns:excerpt=\"http://wordpress.org/export/$wxr_version/excerpt/\" xmlns:content=\"http://purl.org/rss/1.0/modules/content/\" xmlns:wfw=\"http://wellformedweb.org/CommentAPI/\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:wp=\"http://wordpress.org/export/$wxr_version/\">\n" );
			fwrite( $handle, "<channel>\n" );
			fwrite( $handle, "\t<title>" . get_bloginfo_rss( 'name' ) ."</title>\n" );
			fwrite( $handle, "\t<link>" . get_bloginfo_rss( 'url' ) ."</link>\n" );
			fwrite( $handle, "\t<description>" . get_bloginfo_rss( 'description' ) ."</description>\n" );
			fwrite( $handle, "\t<pubDate>" . date( 'D, d M Y H:i:s +0000' ) ."</pubDate>\n" );
			fwrite( $handle, "\t<language>" . get_bloginfo_rss( 'language' ) ."</language>\n" );
			fwrite( $handle, "\t<wp:wxr_version>" . $wxr_version ."</wp:wxr_version>\n" );
			fwrite( $handle, "\t<wp:base_site_url>" . $this->wxr_site_url() ."</wp:base_site_url>\n" );
			fwrite( $handle, "\t<wp:base_blog_url>" . get_bloginfo_rss( 'url' ) ."</wp:base_blog_url>\n" );
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'authors' ) {
			fwrite( $handle, $this->wxr_authors_list() );
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'menus' ) {
			if ( 'all' == $job_object->job[ 'wpexportcontent' ] ) {
				fwrite( $handle, $this->wxr_nav_menu_terms() );
			}
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'posts' ) {
			fwrite( $handle, "\t<generator>http://wordpress.org/?v=" . get_bloginfo_rss( 'version' ) . "</generator>\n" );

			if ( $job_object->steps_data[ $job_object->step_working ]['post_ids'] ) {
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'check' ) {
			...
			if ( extension_loaded( 'simplexml' ) && class_exists( 'DOMDocument' ) ) {
				...
				$job_object->need_free_memory( filesize( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'] ) * 2 );
				$valid = TRUE;
				...
				$success = $dom->loadXML( file_get_contents( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'] ) );
					...
					foreach ( $errors as $error ) {
						switch ( $error->level ) {
							case LIBXML_ERR_WARNING:
								$job_object->log( E_USER_WARNING, sprintf( __( 'XML WARNING (%s): %s', 'backwpup' ), $error->code, trim( $error->message ) ), BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'], $error->line );
								break;
							case LIBXML_ERR_ERROR:
								$job_object->log( E_USER_WARNING, sprintf( __( 'XML RECOVERABLE (%s): %s', 'backwpup' ), $error->code,  trim( $error->message ) ), BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'], $error->line  );
								break;
							case LIBXML_ERR_FATAL:
								$job_object->log( E_USER_WARNING, sprintf( __( 'XML ERROR (%s): %s', 'backwpup' ),$error->code,  trim( $error->message ) ), BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'], $error->line );
								break;
						}
					}
				} else {
		...
		if ( $job_object->steps_data[ $job_object->step_working ]['substep'] == 'compress' ) {
			if ( ! empty( $job_object->job[ 'wpexportfilecompression' ] ) ) {
				...
				try {
					$compress = new BackWPup_Create_Archive( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'] . $job_object->job[ 'wpexportfilecompression' ] );
					if ( $compress->add_file( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'] ) ) {
						unset( $compress );
						unlink( BackWPup::get_plugin_data( 'TEMP' ) . $job_object->steps_data[ $job_object->step_working ]['wpexportfile'] );
