...
class BackWPup_System_Tests_Runner {
	...
	public function run() {
		...
		if ( 200 !== (int) wp_remote_retrieve_response_code( $raw_response )
			&& 204 !== (int) wp_remote_retrieve_response_code( $raw_response )
		) {
			$this->warnings[] = $this->message(
				sprintf(
	...
	/**
	 * Check if scheduling cron seems to be working.
	 *
	 * @return string|null Error message or null when all is ok.
	 */
	 ...
	 *
	 * @param string[] $messages A list of messages to output.
	 *
	 * @return string
	 */
	private function show_messages_list( array $messages ) {
		$output = '<ul>';
